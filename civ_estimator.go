...
// defaultDemoCatalog returns hardcoded demo catalog as fallback
func defaultDemoCatalog() []*ItemCatalogFeatures {
	return []*ItemCatalogFeatures{
...
func demoCIV() {
	cfg, err := LoadCIVConfig("config/civ_config.json")
	if err != nil {
		fmt.Printf("Warning: could not load CIV config: %v\n", err)
		cfg = DefaultCIVConfig()
	}
	estimator := NewCIVEstimator(cfg)

	catalog, err := loadDemoCatalog("data/demo_catalog.json")
	if err != nil {
		fmt.Printf("Warning: could not load demo catalog: %v. Using built-in fallback.\n", err)
...
// Note: main() is commented out to avoid conflict with eom.go's main()
// The demo is run via eom.go's main() function calling demoCIV()
// func main() {
// 	demoCIV()
// }
